...
def clampExclusive(theVal,maxVal):
    if theVal>0:
        return min(theVal,maxVal-1)
    else:
        return max(theVal,-maxVal+1)
...
class NDimTable:
    ...
    def createFromText(infile):
        ...
        ndt.lookupTable=torch.zeros(ndt.nBins)
        if re.search(r"first", infile.readline()):
            valueFirst = True
        else:
            valueFirst = False
        # print valueFirst
    ...
    def whereIs(self, coords):
        ...
        for i in range(self.nDim):
            bin.append(
                int(
                    min(
                        floor((coords[i] - self.minVal[i]) / self.wBin[i]),
                        self.nBins[i] - 1,
                    )
                )
            )
    ...
    def setValueAt(self, bin, val):
        ...
        self.lookupTable[tuple(bin)]=val
    ...
    def bin2index(self, bin):
        ...
        for i in range(self.nDim - 1):
            ...
            if iBin < 0 or iBin >= self.nBins[i]:
                return -1
            idx += iBin
            idx *= self.nBins[i + 1]
        i = self.nDim - 1
    ...
    def bin2index_limit(self, bin):
        ...
        for i in range(self.nDim - 1):
            iBin = self.wrapbin(bin[i], i)
            iBin = self.clampInclusive(
                0, iBin, self.nBins[i] - 1
            )  # clamp to range [ 0, nBins[i] )
            idx += iBin
            idx *= self.nBins[i + 1]
        i = self.nDim - 1
        iBin = self.wrapbin(bin[i], i)
    ...
    def valueAt(self, pt):
        ...
        va_current = [
            0
        ] * self.nDim  # current bin indices we're examining in the 2nd loop below
        ...
        for dim in range(self.nDim):
            ...
            if pt[dim] < va_home_ctr[dim]:
                va_neighbor.append(va_home[dim] - 1)
            else:
                va_neighbor.append(va_home[dim] + 1)
            va_contrib.append(
                abs((pt[dim] - va_home_ctr[dim]) / self.wBin[dim])
            )  # always on [0.0, 0.5]
        ...
        for bin in range(1 << self.nDim):
            ...
            # value += (
                # coeff * self.lookupTable[self.bin2index_limit(va_current)]
            # )
            # print(va_current)
            va_current=[clampExclusive(va_current[i],self.lookupTable.shape[i]) for i in range(self.nDim)]
            value+=(coeff*self.lookupTable[tuple(va_current)])  
            # calc. va_contribution of va_currently selected bin
        return value
