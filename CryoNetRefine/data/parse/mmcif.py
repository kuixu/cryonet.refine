...
def parse_polymer(  # noqa: C901, PLR0915, PLR0912
    ...
    for j, res_name, res, name_to_atom in residue_iterator:
        ...
        # Check if this is the first residue (5'-terminal for DNA/RNA) and add OP3 atom
        is_first_residue = (j == 0)
        if is_first_residue and polymer_type in [gemmi.PolymerType.Dna, gemmi.PolymerType.Rna]:
            # Add OP3 atom to the reference atoms for the first residue
            # Find the P atom to calculate OP3 position
            p_atom = ref_name_to_atom.get("P")
            if p_atom is not None:
                # Create a virtual OP3 atom
                op3_atom = type('Atom', (), {
                    'GetProp': lambda self, prop: 'OP3' if prop == 'name' else None,
                    'GetIdx': lambda self: p_atom.GetIdx(),  # Use P atom's index as reference
                    'GetAtomicNum': lambda self: 8,  # Oxygen
                    'GetFormalCharge': lambda self: -1,  # Phosphate oxygen
        ...
        for ref_atom in ref_atoms:
            ...
            if atom_name == "OP3":
                ...
                if "OP3" in name_to_atom:
                    atom: gemmi.Atom = name_to_atom["OP3"]
                    atom_is_present = True
                    coords = (atom.pos.x, atom.pos.y, atom.pos.z)
                    bfactor = atom.b_iso
                else:
                    # If OP3 not in CIF, calculate position based on P atom
                    if "P" in name_to_atom:
                ...
                # For OXT, check if it exists in the CIF file
                if "OXT" in name_to_atom:
                    atom: gemmi.Atom = name_to_atom["OXT"]
                    atom_is_present = True
                    coords = (atom.pos.x, atom.pos.y, atom.pos.z)
                    bfactor = atom.b_iso
                else:
                    # If OXT not in CIF, calculate position based on C atom
                    if "C" in name_to_atom:
                        ...
                        coords = (0, 0, 0)
                        bfactor = 0
            elif atom_name in name_to_atom:
