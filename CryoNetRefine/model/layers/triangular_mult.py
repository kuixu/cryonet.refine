...
from CryoNetRefine.model.layers import initialize as init
...
def kernel_triangular_mult(
    ...
    try:
        ...
        # Fallback to PyTorch implementation if CUDA kernels are not available
        raise ImportError("CUDA kernels not available. Please set use_kernels=False or install compatible cuequivariance_ops.")
...
class TriangleMultiplicationOutgoing(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor, use_kernels: bool = False) -> Tensor:
        ...
        if use_kernels:
            ...
            print(f"Debug - p_in_weight shape: {self.p_in.weight.shape}")
            print(f"Debug - g_in_weight shape: {self.g_in.weight.shape}")
            print(f"Debug - p_out_weight shape: {self.p_out.weight.shape}")
            print(f"Debug - g_out_weight shape: {self.g_out.weight.shape}")
