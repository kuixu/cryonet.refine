...
from CryoNetRefine.model.layers.attention import AttentionPairBias
from CryoNetRefine.model.layers.dropout import get_dropout_mask
from CryoNetRefine.model.layers.transition import Transition
from CryoNetRefine.model.layers.triangular_attention.attention import (
    TriangleAttentionEndingNode,
    TriangleAttentionStartingNode,
)
from CryoNetRefine.model.layers.triangular_mult import (
...
def log_cuda_mem(tag: str):
    if torch.cuda.is_available():
        ...
        print(f"[MEM][Protenix] {tag} | alloc={cur:.1f}MB reserved={res:.1f}MB peak={peak:.1f}MB")
    else:
        print(f"[MEM][Protenix] {tag} | CUDA not available")
...
class PairformerNoSeqLayer(nn.Module):
    ...
    def forward(
        ...
        else: print(f"[MEM][Protenix] CUDA not available")
        # tri_mul_out
        dropout = get_dropout_mask(self.dropout, z, self.training)
        z = z + dropout * self.tri_mul_out(
            z, mask=pair_mask, use_kernels=use_cuequiv_mul or use_kernels
